                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" class="form-control" id="item_order" name="item_order" 
                                   value="{{ old('item_order', isset($banner) ? $banner->items->count() + 1 : 1) }}" required min="1">
    ...
    if (select2Elements.length && typeof $.fn.select2 !== 'undefined') {
        select2Elements.forEach(function(element) {
            $(element).select2({
                ...
                dropdownParent: $(element).closest('.modal').length ? $('#itemModal') : document.body
            });
        });
    }
}
...
function openItemModal() {
    ...
    if (itemOrderInput) {
        const itemCount = parseInt(document.getElementById('itemsContainer')?.querySelectorAll('.item-card').length) || 0;
...
function editItem(itemId) {
    ...
    .then(response => {
        ...
        if (itemIsActive) itemIsActive.checked = response.is_active;
        ...
        setTimeout(() => {
            ...
            if (promoSelect && response.promo_codes) {
                $(promoSelect).val(response.promo_codes).trigger('change');
            }
        }, 100);
