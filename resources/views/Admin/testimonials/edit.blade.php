    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb bg-transparent p-0 mb-4">
                ...
                <li class="breadcrumb-item active text-white">تعديل رأي: {{ $testimonial->name }}</li>
            </ol>
        </nav>
        ...
        <div class="info-bar">
            <div class="info-item">
                ...
                <span>العميل:</span>
                <span class="info-value">{{ $testimonial->name }}</span>
            </div>
            <div class="info-item">
                <i class="fas fa-star" style="color: var(--star-color);"></i>
                <span>التقييم:</span>
                <span class="info-value">{{ $testimonial->rating }}/5</span>
            </div>
            <div class="info-item">
                <i class="fas fa-calendar-alt"></i>
                <span>تاريخ الإنشاء:</span>
                <span class="info-value">{{ $statistics['created_at'] }}</span>
            </div>
            <div class="info-item">
                <i class="fas fa-clock"></i>
                <span>آخر تحديث:</span>
                <span class="info-value">{{ $statistics['updated_at'] }}</span>
            </div>
            <div class="info-item">
                <i class="fas fa-font"></i>
                <span>الكلمات:</span>
                <span class="info-value">{{ $statistics['word_count'] }} كلمة</span>
            </div>
        </div>
        ...
        <div class="alert-guide">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <h6 class="mb-1">تعديل رأي العميل</h6>
                    <p class="mb-0 text-muted">
                        <i class="fas fa-info-circle me-1"></i> يمكنك تعديل جميع بيانات الرأي |
                        <i class="fas fa-image me-1"></i> تغيير أو حذف الصورة |
                        <i class="fas fa-eye me-1"></i> معاينة التغييرات قبل الحفظ
                    </p>
                </div>
            </div>
        </div>
            ...
            <div class="step-card step-1">
                <div class="step-header">
                    ...
                    <div>
                        ...
                        <p class="step-description">تعديل اسم العميل والمدينة والصورة</p>
                    </div>
                </div>
                        ...
                        <div class="avatar-upload-container">
                            <div class="avatar-preview-wrapper" onclick="document.getElementById('avatar').click()">
                                @if ($testimonial->avatar)
                                    <img id="avatarPreview" src="{{ asset('storage/' . $testimonial->avatar) }}" alt="{{ $testimonial->name }}">
                            ...
                            <div class="avatar-actions">
                                ...
                                @if ($testimonial->avatar)
                                    <button type="button" class="btn btn-sm btn-outline-danger" onclick="removeAvatar()" id="removeAvatarBtn">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control form-control-lg" id="name" name="name"
                                    value="{{ old('name', $testimonial->name) }}" placeholder="أدخل اسم العميل"
                                    oninput="updatePreview()" required>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label for="city" class="form-label">المدينة</label>
                                <input type="text" class="form-control form-control-lg" id="city" name="city"
                                    value="{{ old('city', $testimonial->city) }}" placeholder="مثال: الرياض، جدة، القاهرة..."
            ...
            <div class="step-card step-2" style="display: none;">
                <div class="step-header">
                    ...
                    <div>
                        ...
                        <p class="step-description">تعديل التقييم والمراجعة</p>
                    </div>
                </div>
                    ...
                    <div class="star-rating-input">
                        @for ($i = 5; $i >= 1; $i--)
                            <input type="radio" id="star{{ $i }}" name="rating" value="{{ $i }}"
                                {{ old('rating', $testimonial->rating) == $i ? 'checked' : '' }} onchange="updateRating(this.value)">
                            <label for="star{{ $i }}" title="{{ $i }} نجوم">
                                <i class="fas fa-star"></i>
                            </label>
                        @endfor
                    </div>
                    <div class="rating-text {{ getRatingClass(old('rating', $testimonial->rating)) }}" id="ratingText">
                        {{ getRatingText(old('rating', $testimonial->rating)) }}
                    </div>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea class="form-control" id="review" name="review" rows="5"
                        ...
                        maxlength="500" required>{{ old('review', $testimonial->review) }}</textarea>
                    <div class="char-counter" id="charCounter">
                        <span id="charCount">{{ strlen(old('review', $testimonial->review)) }}</span>/500 حرف
                    </div>
                </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <div class="toggle-container mt-2">
                            ...
                            <span class="toggle-label" id="statusLabel">
                                {{ old('is_active', $testimonial->is_active) ? 'نشط' : 'غير نشط' }}
                            </span>
                        </div>
                    </div>
                </div>
            ...
            <div class="step-card step-3" style="display: none;">
                <div class="step-header">
                    ...
                    <div>
                        ...
                        <p class="step-description">راجع التغييرات قبل الحفظ</p>
                    </div>
                </div>
                ...
                <div class="mb-4">
                    <h6 class="mb-3">
                        ...
                        معاينة مباشرة - هكذا سيظهر الرأي بعد التعديل
                    </h6>
                    <div class="col-md-6 mx-auto">
                        <div class="preview-card">
                            <div class="preview-avatar" id="previewAvatarContainer">
                                @if ($testimonial->avatar)
                                    <img id="previewAvatarImg" src="{{ asset('storage/' . $testimonial->avatar) }}" alt="{{ $testimonial->name }}">
                                    <i class="fas fa-user" id="previewAvatarIcon" style="display: none;"></i>
                                @else
                            ...
                            <div class="preview-review" id="previewReview">
                                "{{ old('review', $testimonial->review) }}"
                            </div>
                            <div class="preview-name" id="previewName">{{ old('name', $testimonial->name) }}</div>
                            <div class="preview-city" id="previewCity">{{ old('city', $testimonial->city) }}</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="summary-card">
                    <h6 class="mb-3"><i class="fas fa-info-circle me-2"></i>ملخص التعديلات</h6>

                    <div class="summary-row">
                        <span class="summary-label">اسم العميل:</span>
                        <span class="summary-value" id="summaryName">{{ old('name', $testimonial->name) }}</span>
                    </div>
                    <div class="summary-row">
                        <span class="summary-label">المدينة:</span>
                        <span class="summary-value" id="summaryCity">{{ old('city', $testimonial->city) ?: '-' }}</span>
                    </div>
                    <div class="summary-row">
                        <span class="summary-label">التقييم:</span>
                        <span class="summary-value" id="summaryRating">{{ old('rating', $testimonial->rating) }}/5 ⭐</span>
                    </div>
                    <div class="summary-row">
                        <span class="summary-label">عدد كلمات المراجعة:</span>
                        <span class="summary-value" id="summaryWordCount">{{ str_word_count(old('review', $testimonial->review)) }} كلمة</span>
                    </div>
                    <div class="summary-row">
                        <span class="summary-label">الحالة:</span>
                        <span class="summary-value" id="summaryStatus">{{ old('is_active', $testimonial->is_active) ? 'نشط' : 'غير نشط' }}</span>
                    </div>
                    <div class="summary-row">
                        <span class="summary-label">ترتيب العرض:</span>
                        <span class="summary-value" id="summaryOrder">{{ old('sort_order', $testimonial->sort_order) }}</span>
                    </div>
                    <div class="summary-row">
                        <span class="summary-label">الصورة:</span>
                        <span class="summary-value" id="summaryAvatar">
                            @if ($testimonial->avatar)
                                موجودة
                            @else
                                لا توجد صورة
...
@section('js')
    ...
    <script>
        ...
        $('#testimonialForm').on('submit', function(e) {
            ...
            Swal.fire({
                title: 'حفظ التعديلات؟',
                text: 'هل أنت متأكد من حفظ التعديلات على هذا الرأي؟',
                icon: 'question',
                ...
                if (result.isConfirmed) {
                    $('#testimonialForm').off('submit').submit();
                }
            });
        });
        ...
        function updateSummary() {
            ...
            // Avatar status
            const removeAvatar = $('#remove_avatar').val() === '1';
            const hasNewAvatar = $('#avatarPreview').attr('src') && $('#avatarPreview').attr('src') !== '';

            if (removeAvatar) {
                $('#summaryAvatar').text('سيتم حذفها');
            } else if (hasNewAvatar) {
                $('#summaryAvatar').text('تم تغييرها');
            } else {
                @if ($testimonial->avatar)
                    $('#summaryAvatar').text('موجودة (لم تتغير)');
                @else
                    $('#summaryAvatar').text('لا توجد صورة');
                @endif
            }
        }
