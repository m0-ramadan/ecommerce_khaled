    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.stores.update', $store->id) }}">
                ...
                @method('PUT')
                <div class="mb-3">
                    <chet for="name" class="form-label">الاسم</label>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="desc" name="desc">{{ old('desc', $store->desc) }}</textarea>
                    @error('desc')
                    <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control" id="quantity" name="quantity"
                        value="{{ old('quantity', $store->quantity) }}" required>
                    @error('quantity')
                    <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                <div class="mb-3">
                    ...
                    <select class="form-control" id="shipment_id" name="shipment_id">
                        ...
                        <option value="{{ $shipment->id }}" {{ old('shipment_id', $store->shipment_id) == $shipment->id
                            ? 'selected' : '' }}>
                            {{ $shipment->code }}
                        </option>
                        @endforeach
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control" id="branch_id" name="branch_id">
                        ...
                        <option value="{{ $branch->id }}" {{ old('branch_id', $store->branch_id) == $branch->id
                            ? 'selected' : '' }}>
                            {{ $branch->name }}
                        </option>
                        @endforeach
