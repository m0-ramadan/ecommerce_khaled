                    ...
                    <div class="row">
                                                    ...
                                                    <td class="center">
                                                        <button class="btn btn-success action-btn" data-bs-toggle="modal"
                                                            ...
                                                            data-content='{{ json_encode(
                                                                $shipment->contents->map(function ($content) use ($trip) {
                                                                        $tripContent = $content->tripShipmentContents->where('trip_id', $trip->id)->first();
                                                                        return [
                                                                            'id' => $content->id,
                                                                            'code' => $content->code ?? '--',
                                                                            'name' => $content->name ?? '--',
                                                                            ...
                                                                            'selected_quantity' => $tripContent ? $tripContent->quantity : 0,
                                                                        ];
                                                                    })->toArray(),
                                                            ) }}'>
                        ...
                        <div class="col-md-12 mt-4">
                            ...
                            <div class="table-responsive">
                                <table class="content-table" id="selected-contents-table">
                                    ...
                                    <tbody id="selected-contents-body">
                                        @foreach ($trip->contents as $content)
                                            <tr data-content-id="{{ $content->id }}">
                                                <td class="center">{{ $content->code ?? '--' }}</td>
                                                <td class="center">{{ $content->name ?? '--' }}</td>
                                                <td class="center">{{ $content->shipment->code ?? '--' }}</td>
                                                <td class="center">{{ $content->remaining ?? 0 }}</td>
                                                <td class="center">
                                                    <input type="number" name="contents[{{ $content->id }}][id]"
                                                        value="{{ $content->id }}" hidden>
                                                    <input type="number" name="contents[{{ $content->id }}][quantity]"
                                                        value="{{ $content->pivot->quantity }}"
                                                        class="form-control content-quantity" min="1"
                                                        max="{{ $content->remaining }}"
                                                        data-max="{{ $content->remaining }}" required>
                                                    <span class="invalid-feedback">الكمية يجب أن تكون بين 1 و
                                                        {{ $content->remaining }}</span>
                                                </td>
                                                <td class="center">
                                                    <button type="button"
                                                        class="btn btn-danger btn-sm remove-content">إزالة</button>
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                            ...
                            <select class="form-control select2 @error('representative_id') is-invalid @enderror"
                                ...
                                @foreach ($representatives as $representative)
                                    <option value="{{ $representative->id }}"
                                        {{ $trip->representative_id == $representative->id ? 'selected' : '' }}>
                                        {{ $representative->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select class="form-control select2 @error('branch_from') is-invalid @enderror"
                                ...
                                @foreach ($branches as $branch)
                                    <option value="{{ $branch->id }}"
                                        {{ $trip->branches_from == $branch->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-4 mt-3">
                            ...
                            <select class="form-control select2 @error('branch_to') is-invalid @enderror" name="branch_to"
                                ...
                                @foreach ($branches as $branch)
                                    <option value="{{ $branch->id }}"
                                        {{ $trip->branches_to == $branch->id ? 'selected' : '' }}>
                                        {{ $branch->name }} ({{ $branch->region->region_ar ?? 'غير محدد' }})
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-4 mt-3">
                            ...
                            <input type="number" step="0.01"
                                ...
                                id="value_drive" value="{{ old('value_drive', $trip->value_drive) }}" required>
                            @error('value_drive')
                                <span class="invalid-feedback text-black font-weight-bold text-capitalize" role="alert">
                                    {{ $message }}
                                </span>
                        ...
                        <div class="col-md-4 mt-3">
                            ...
                            <input type="number" step="0.01"
                                ...
                                id="expense_value" value="{{ old('expense_value', $trip->expense_value) }}">
                            @error('expense_value')
                                <span class="invalid-feedback text-black font-weight-bold text-capitalize" role="alert">
                                    {{ $message }}
                                </span>
                        ...
                        <div class="col-md-4 mt-3">
                            ...
                            <input type="number" step="0.01"
                                ...
                                id="refund_value" value="{{ old('refund_value', $trip->refund_value) }}" readonly>
                            @error('refund_value')
                                <span class="invalid-feedback text-black font-weight-bold text-capitalize" role="alert">
                                    {{ $message }}
                                </span>
                        ...
                        <div class="col-md-4 mt-3">
                            ...
                            <input class="form-control @error('notes') is-invalid @enderror" name="notes"
                                id="notes" type="text" placeholder="ملاحظات"
                                value="{{ old('notes', $trip->notes) }}" data-parsley-trigger="change">
                            @error('notes')
                                <span class="invalid-feedback text-black font-weight-bold text-capitalize" role="alert">
                                    {{ $message }}
                                </span>
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.action-btn').on('click', function() {
                ...
                try {
                    ...
                    if (contents && Array.isArray(contents) && contents.length > 0) {
                                ...
                                <tr data-content-id="${content.id}">
                                    ...
                                    <td class="center">
                                        <input type="number" class="form-control content-quantity" min="1" max="${content.remaining || 0}"
                                               value="${isChecked ? $(`#selected-contents-body input[name="contents[${content.id}][quantity]"]`).val() || content.selected_quantity || '' : content.selected_quantity || ''}"
                        ...
                        $modalBody.append(
                            '<tr><td colspan="6" class="center">لا توجد محتويات متاحة</td></tr>');
                    }
                } catch (error) {
            ...
            $('#add-selected-contents').on('click', function() {
                try {
                    ...
                    $modalRows.each(function() {
                        ...
                        const shipmentCode = $(this).find('td:eq(3)').text();
                        const remaining = parseInt($(this).find('td:eq(4)').text()) || 0;

                        console.log(
                            `Processing contentId: ${contentId}, quantity: ${quantity}, remaining: ${remaining}`
                        );
            ...
            // Validate quantity inputs in selected contents table
            ...
            // Restore old input for selected contents (in case of validation errors)
            @if (old('contents'))
                    ...
                    @if ($content)
                        try {
                            const content = {
                                ...
                                code: '{{ $content->code ?? '--' }}',
                                name: '{{ $content->name ?? '--' }}',
                                shipmentCode: '{{ $content->shipment->code ?? '--' }}',
                                remaining: {{ $content->remaining ?? 0 }},
