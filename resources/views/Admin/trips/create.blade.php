    ...
    <div class="container">
        ...
        <div class="card">
            <div class="card-body">
                ...
                <form action="{{ route('admin.trips.store.representative') }}" method="POST" data-parsley-validate>
                    @csrf
                    <div class="row">
                        <!-- Representative -->
                        ...
                        <div class="col-md-6 mt-4">
                            ...
                            <select style="direction: rtl; text-align: right;"
                                ...
                                @foreach ($shipments as $shipment)
                                    <option value="{{ $shipment->id }}" @if (is_array(old('shipments')) && in_array($shipment->id, old('shipments'))) selected @endif>

                                        {{ $shipment->code }}
                                        ({{ $shipment->branchFrom?->name ?? '--' }} -
                                        {{ $shipment->branchTo?->name ?? '--' }})
                                        (سعر الشحنة: {{ $shipment->price }})
                                        {{-- ({{ 'نوع المركبة: ' . ($shipment->vehicleType?->name ?? '--') }}) --}}
